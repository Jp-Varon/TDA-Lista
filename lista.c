...
nodo_t* borrar_nodo(nodo_t * nodo_actual){
	if(!nodo_actual->siguiente){
		free(nodo_actual);
		nodo_actual = NULL;
		return nodo_actual;
	}
	nodo_actual->siguiente = borrar_nodo(nodo_actual->siguiente);
	return nodo_actual; 
}

int lista_borrar(lista_t* lista){
	if(!lista || !lista->nodo_inicio || lista->cantidad == 0){
		return -1;
	}
	lista->nodo_inicio = borrar_nodo(lista->nodo_inicio);
	if(lista->nodo_inicio){
		lista->nodo_fin = ultimo_nodo(lista->nodo_inicio);
	}else lista->nodo_fin = NULL; 
	(lista->cantidad)--;
	return 0;
}
